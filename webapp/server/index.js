...
async function genImageForScene(c, scenes, i, promptOverride, frameRole) {
  ...
  const frameDir = frameRole === 'end'
    ? `[FRAME = END KEYFRAME — a slightly LATER moment of the SAME shot] The clip animates from the start frame to this one, so it differs — but ONLY in her FACIAL EXPRESSION (now RESOLVED to its payoff — the earned half-smile, the settled "oh, that's why") and a SMALL natural shift${motion ? ` (${motion})` : ''}. CRITICAL — keep it the SAME shot: the product stays in the SAME hand, in roughly the same place; do NOT switch the product to the other hand, do NOT mirror/flip the composition, do NOT move to a different pose or relocate the product. Same background, same wardrobe, same framing, same hands — only the expression (and a subtle shift) changes.`
    : `[FRAME = START KEYFRAME — the FIRST frame of the clip] Render the OPENING moment of this shot: the expression just BEGINNING (the first flicker), the product held naturally. Note which hand holds the product — the end frame will keep it in that SAME hand.`
  const prompt = [
    scenePrompt,
    frameDir,
