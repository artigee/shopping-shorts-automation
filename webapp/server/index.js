...
async function genImageForScene(c, scenes, i, promptOverride, frameRole) {
  ...
  const frameDir = frameRole === 'end'
    ? `[FRAME = END KEYFRAME — a LATER moment of the SAME shot] The clip animates from the start frame to this one, so there IS real motion: her pose, gesture, gaze and expression move through the action${motion ? ` (${motion})` : ''}, settling into the resolved/payoff expression. Keep the shot's identity the same: same background, same wardrobe, same framing, same lighting. HARD RULE — the product stays in the SAME hand as the start frame: NEVER switch the product to the other hand, and NEVER mirror or flip the composition. Her free hand, head and body may move naturally with the motion, but the product-holding hand does not swap sides.`
    : `[FRAME = START KEYFRAME — the FIRST frame of the clip] Render the OPENING moment of this shot: the expression just BEGINNING (the first flicker), the product held naturally. Note which hand holds the product — the end frame keeps the product in that SAME hand (motion may change the pose, but never the product hand).`
  const prompt = [
    scenePrompt,
    frameDir,
