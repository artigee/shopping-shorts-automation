...
ensureColumn('reels', 'score2', 'REAL')            // v2 점수 (계정 대비 성과 정규화) — v1(score)와 병행 비교
ensureColumn('reels', 'product_focus', 'TEXT')     // AI 분류: single | roundup | non_shoppable
ensureColumn('contents', 'character_ref', 'TEXT')    // 캐릭터(인물) 레퍼런스 — 모든 씬 이미지에 동일 인물 적용 (url 또는 hfmedia:)
ensureColumn('contents', 'character_element', 'TEXT') // 히긱스필드 element로 지정된 캐릭터 JSON { id, name } — 생성 시 프롬프트에 <<<id>>> 주입 (정체성 고정, 규칙 대체)
...
ensureColumn('contents', 'graph_state', 'TEXT')      // 노드 그래프 레이아웃 보존 JSON { pos, extraNodes, edges, data } — 수동 노드/연결/위치 유지
