...
export async function genImageViaCLI({ prompt, productImageUrls = [], productImageUrl, characterRef, envRef, productName, dimensions }) {
  ...
  const scaleFix = `SCALE CORRECTION (do this first, silently): The product is "${(productName || 'the product').replace(/"/g, "'")}"${dimensions ? ` with real-world dimensions ${dimensions}` : ''}. Rewrite the scene prompt below so the product appears at its TRUE real-world size. If the prompt frames a physically large product (bed, furniture, large item) as a small handheld "compact pouch/bag" in a hand close-up, FIX the framing: widen the shot and add a human/room/furniture scale anchor so the real size reads correctly. Keep the user's scene intent and setting. Keep vertical 9:16, photorealistic. CRITICAL RULES: (1) ABSOLUTELY NO text — no letters, words, numbers, captions, labels, signs, logos, or watermarks anywhere. (2) Keep the mood warm, bright, clean and aspirational — even a problem/"before" beat stays light and relatable, NEVER genuinely distressing (no crying/upset child, no exhausted despair, no bleak or messy-despair framing, no dark grime).`
