...
export async function genImageViaCLI({ prompt, productImageUrls = [], productImageUrl, characterRef, envRef, sceneRef, productName, dimensions }) {
  ...
  const labeled = [
    ...(sceneRef ? [{ r: sceneRef, role: 'SCENE ANCHOR (HIGHEST PRIORITY — this is the START frame of the SAME clip). Keep the SAME background/setting, wardrobe, framing/distance/angle and lighting as this image. Her pose, gesture and expression MAY change with the motion (this is the end frame). HARD RULE: the product stays in the SAME hand as this image — NEVER switch the product to the other hand, and NEVER mirror or flip the shot. Only the pose/expression evolve; the product hand and the scene identity stay put.' }] : []),
