...
export async function genImageViaCLI({ prompt, productImageUrls = [], productImageUrl, characterRef, envRef, sceneRef, productName, dimensions }) {
  ...
  const labeled = [
    ...(sceneRef ? [{ r: sceneRef, role: 'SCENE ANCHOR (HIGHEST PRIORITY — this is the START frame of the SAME clip). Reproduce this image EXACTLY: same background/setting, same wardrobe, same framing/distance/angle, same lighting, same hands, and the product held in the SAME hand and same place. Do NOT mirror or flip the image, and NEVER switch the product to the other hand. The ONLY thing that changes is her FACIAL EXPRESSION (and at most a subtle shift). Everything else is identical to this image so the clip morphs cleanly.' }] : []),
