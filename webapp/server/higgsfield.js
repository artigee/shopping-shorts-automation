...
export async function genVideoViaCLI({ imageUrl, prompt, duration = 5 }) {
...
1) Call media_import_url {"url":"${imageUrl}","type":"image"} and keep the media_id.
2) Call generate_video {"model":"kling3_0_turbo","prompt":"${safe}","aspect_ratio":"9:16","duration":${duration},"medias":[{"value":"<media_id>","role":"start_image"}]}.
...
export async function genImageViaCLI({ prompt, productImageUrls = [], productImageUrl, characterRef, envRef, productName, dimensions }) {
  ...
  const scaleFix = `SCALE CORRECTION (do this first, silently): The product is "${(productName || 'the product').replace(/"/g, "'")}"${dimensions ? ` with real-world dimensions ${dimensions}` : ''}. Rewrite the scene prompt below so the product appears at its TRUE real-world size. If the prompt frames a physically large product (bed, furniture, large item) as a small handheld "compact pouch/bag" in a hand close-up, FIX the framing: widen the shot and add a human/room/furniture scale anchor so the real size reads correctly. Keep the user's scene intent and setting. Keep vertical 9:16, photorealistic. CRITICAL RULES: (1) The PRODUCT's own design and branding/logo SHOULD render accurately from the reference (do not blank it out) — but keep it at a natural distance so fine print isn't legible; NO overlay captions/subtitles, NO added on-screen text, and NO random or garbled signage/paragraph text on other objects. (2) Keep the mood warm, bright, clean and aspirational — even a problem/"before" beat stays light and relatable, NEVER genuinely distressing (no crying/upset child, no exhausted despair, no bleak or messy-despair framing, no dark grime).`
